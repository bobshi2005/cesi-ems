package com.cesi.peakvalley.service.impl;


import cn.hutool.core.date.DateUtil;
import com.cesi.common.enums.ElectricityTypeEnum;
import com.cesi.common.enums.TimeType;
import com.cesi.common.utils.DateUtils;
import com.cesi.costmanagement.mapper.CostPriceRelevancyMapper;
...
import com.cesi.peakvalley.domain.vo.peakvalley.*;
...
public class EnergyUsedElectricityServiceImpl implements IEnergyUsedElectricityService {
    ...
    public List<PeakValleyHourDataVO> getDataStatistics(ElectricityDataItemListDTO dto) {
        ...
        Map<String, List<com.cesi.peakvalley.domain.EnergyUsedElectricity>> electricityDataMap = new HashMap<>();
        // 查询点位信息
        ...
        if (CollectionUtils.isNotEmpty(nodeIndexInfoList)) {
            ...
            List<com.cesi.peakvalley.domain.EnergyUsedElectricity> dataItemList = electricityDataItemMapper.getDataStatistics(indexSet, startTime, endTime, timeType);
        ...
        while (!startTime.after(endTime)) {
            ...
            List<com.cesi.peakvalley.domain.EnergyUsedElectricity> dataItemList = electricityDataMap.get(mapKey);
            ...
            if (CollectionUtils.isNotEmpty(dataItemList)) {
                for (com.cesi.peakvalley.domain.EnergyUsedElectricity energyUsedElectricity : dataItemList) {
    ...
    public PeakValleyDayVO segmentAnalysisDay(PeakValleyDTO dto) {
        ...
        if (CollectionUtils.isNotEmpty(nodeIndexInfoList)) {
            Set<String> indexSet = nodeIndexInfoList.stream().map(ModelNodePointInfo::getIndexId).collect(Collectors.toSet());
            // 根据小时数据计算天的数据
            List<com.cesi.peakvalley.domain.EnergyUsedElectricity> dataItemList = electricityDataItemMapper.getDataStatistics(indexSet, startTime, endTime, TimeType.HOUR.name());
        ...
        while (!startTime.after(endTime)) {
            ...
            List<com.cesi.peakvalley.domain.EnergyUsedElectricity> dataItemList = new ArrayList<>();
            for (Map.Entry<String, List<com.cesi.peakvalley.domain.EnergyUsedElectricity>> entry : electricityDataMap.entrySet()) {
                ...
                if ((DateUtils.parseDate(key).after(startTime) || DateUtils.parseDate(key).equals(startTime)) && DateUtils.parseDate(key).before(nextTime)) {
                    List<com.cesi.peakvalley.domain.EnergyUsedElectricity> list = entry.getValue();
