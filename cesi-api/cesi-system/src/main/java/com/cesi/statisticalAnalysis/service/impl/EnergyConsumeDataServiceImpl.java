package com.cesi.statisticalAnalysis.service.impl;
...
import com.cesi.carbonemission.domain.CarbonEmission;
...
import com.cesi.common.constant.DateTimeUtil;
import lombok.AllArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.apache.commons.lang3.ObjectUtils;
import com.cesi.statisticalAnalysis.domain.vo.*;
...
public class EnergyConsumeDataServiceImpl implements IEnergyConsumeDataService {
    ...
    private BigDecimal getEnergyUnitCostTrendAnalysisValueInfo(String timeType, Date bsTime, Date endTime, BigDecimal totalCost,
        ...
        switch (sysEnergyInfo.getEnersno()) {
            case "electric":
                List<com.cesi.peakvalley.domain.EnergyUsedElectricity> energyUsedElectricities = peakValleyMapper.getDataStatistics(nodeIndices.stream().map(NodePoint::getPointId).collect(Collectors.toSet()), bsTime, endTime, TimeTypeConst.TIME_TYPE_HOUR);
                costValue = energyUsedElectricities.stream().map(com.cesi.peakvalley.domain.EnergyUsedElectricity::getCost).reduce(BigDecimal.ZERO, BigDecimal::add);
                accumulationValue = energyUsedElectricities.stream().map(com.cesi.peakvalley.domain.EnergyUsedElectricity::getElectricity).reduce(BigDecimal.ZERO, BigDecimal::add);
                break;
            default:
    ...
    public List<EnergyConsumeTrendDetailItem> listEnergyCostTrendDetail(String timeCode, String timeType, String modelCode, String energyType) {
        ...
        for (SysEnergy sysEnergyInfo : sysEnergies) {
            ...
            switch (sysEnergyInfo.getEnersno()) {
                case "electric":
                    List<com.cesi.peakvalley.domain.EnergyUsedElectricity> energyUsedElectricities = peakValleyMapper.getCostTrends(startTime, endTime, queryTimeType, nodeId, sysEnergyInfo.getEnersno());
    ...
    public List<EnergyTypeValueContrastedVO> listEnergyTypeYoyInfo(QueryCompareRequest req, String comparisonType) {
        ...
        switch (sysEnergyInfo.getEnersno()) {
            case "electric":
                List<com.cesi.peakvalley.domain.EnergyUsedElectricity> energyUsedElectricities = peakValleyMapper.getCostTrends(startTime, endTime, queryTimeType, nodeId, sysEnergyInfo.getEnersno());
                List<com.cesi.peakvalley.domain.EnergyUsedElectricity> lastDataItemList = peakValleyMapper.getCostTrends(lastBeginTime, lastEndTime, queryTimeType, nodeId, sysEnergyInfo.getEnersno());
