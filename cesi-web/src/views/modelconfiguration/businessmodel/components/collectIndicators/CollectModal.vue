  ...
  collectList(currentNode.value.id, { ...queryParams.value, ...form.value }).then((response) => {
    ...
    nextTick(function () {
      collectListTable.value.data.forEach((rowData) => {
        let exist = selectedCollect.value.filter((f) => f.pointId === rowData.pointId).length
        if (exist > 0) {
...
function handleSelectionAllChange(selection) {
  if (formatterSelect(selection, tableData.value)) {
    selection.forEach((row) => {
      let rowStatus = selectedCollect.value.filter((f) => f.pointId == row.pointId).length
      if (rowStatus === 0) {
...
function handleSelectChange(selection, row) {
  ...
  if (rowStatus > 0) {
    ...
    selectedCollect.value = selectedCollect.value.filter((f) => f.pointId !== row.pointId)
  }
}
