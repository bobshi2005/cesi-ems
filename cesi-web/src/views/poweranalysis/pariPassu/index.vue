        ...
        <div class="content-area" v-loading="loading">
          ...
          <div class="chart-card">
            <div class="chart-card-head">
              <span>{{ queryParams.nodeName }}-能耗同比分析</span>
              <span class="head-tag">同比</span>
            </div>
          ...
          <div class="chart-card">
            <div class="chart-card-head">
              <span>{{ queryParams.nodeName }}-能耗同比分析表-{{ queryParams.enername }}</span>
            </div>
            <div class="table-box">
...
<script setup name="pariPassu">
import { querySameCompareList } from "@/api/energyAnalysis/energyAnalysis"
  ...
  querySameCompareList(
    ...
    if (res.data) {
      res.data.forEach((item) => {
        xdata.push(
          proxy.dayjs(item.currentTime).format(
            queryParams.value.timeType == "YEAR"  ? "MM月" :
            queryParams.value.timeType == "MONTH" ? "DD日" : "HH时"
          )
        )
        yvalue.push(item.currentValue   || 0)
        ycompareValue.push(item.contrastValues || 0)
        yqoq.push(item.ratio || 0)
      })
    }
    ...
    setTimeout(() => {
      ...
      echarts.init(dom).setOption({
        ...
        grid: { top: 55, left: "4%", right: "5%", bottom: 10, containLabel: true },
        ...
        yAxis: [
          ...
          {
            type: "value", name: "同比(%)", alignTicks: true,
            nameTextStyle: { color: textColor, fontSize: 12, padding: [0, 0, 5, 0] },
            axisLine: { show: false }, axisTick: { show: false },
...
function resetQuery() {
  if (energyTypeList.value?.[0]) {
    queryParams.value.energyType = energyTypeList.value[0].enersno
    queryParams.value.enername   = energyTypeList.value[0].enername
    queryParams.value.muid       = energyTypeList.value[0].muid
  }
  queryParams.value.analysisType = "YOY"
  handleTimeType(period.value[1]?.value || "MONTH")
}
</script>
