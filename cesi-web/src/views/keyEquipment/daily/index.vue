<template>
  <div class="app-container daily-page">
    <!-- 筛选栏 -->
...
function getList() {
  ...
  loading.value = true
  keyEquipmentApi
    .dailyList({ ...queryParams.value, timeType: "HOUR" })
    .then((res) => {
      loading.value = false
...
function selectChange(row) {
  ...
  keyEquipmentApi.dailyChart(queryParams.value).then((res) => {
    const xData = [], yData = []
    let title = ""
    res.data.forEach((item) => {
      yData.push(numFilter(item.value))
      xData.push(item.timeCode.slice(-2) + "时")
...
keyEquipmentApi.getPointFacility().then((res) => { facilityList.value = res.data })
