<template>
  <div class="app-container year-page">
    <!-- 筛选栏 -->
...
function getTime() {
  queryParams.value.dataTime = proxy.dayjs(new Date()).format("YYYY")
}
...
// 年视图 API 需要将 "YYYY" 补全为 "YYYY-MM-DD" 格式传入
function apiParams() {
  return {
    ...queryParams.value,
...
function selectChange(row) {
  ...
  keyEquipmentApi.yearChart(apiParams()).then((res) => {
    const xData = [], yData = []
    let title = ""
    res.data.forEach((item) => {
      yData.push(numFilter(item.value))
      xData.push(item.timeCode.slice(-2) + "月")
